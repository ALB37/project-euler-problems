...
// In the 20×20 grid below, four numbers along a diagonal line have been marked with stars.
...
const largestProductInAGrid = matrix => {
  let largestProduct = 0;
  // horizontal
  for (let i = 0; i < matrix.length; i++){
    for (let j = 0; j < matrix[i].length - 3; j++){
      let currentProduct = matrix[i][j] * matrix[i][j + 1] * matrix[i][j + 2] * matrix[i][j + 3];
  ...
  for (let i = 0; i < matrix.length - 3; i++) {
    for (let j = 0; j < matrix[i].length; j++) {
      let currentProduct = matrix[i][j] * matrix[i + 1][j] * matrix[i + 2][j] * matrix[i + 3][j];
      if (currentProduct > largestProduct) {
        largestProduct = currentProduct;
      }
    }
  ...
  for (let i = 0; i < matrix.length - 3; i++) {
    for (let j = 0; j < matrix[i].length - 3; j++) {
      let currentProduct = matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3];
      if (currentProduct > largestProduct) {
        largestProduct = currentProduct;
      }
    }
...
console.log(largestProductInAGrid(TWENTY_BY_TWENTY_MATRIX));